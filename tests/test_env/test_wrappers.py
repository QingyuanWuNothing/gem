...
def test_encapsulate_wrapper():
    ...
    boxed_wrapper = get_wrapper_fns("answer_boxed")[0]
    ...
    env = DummyEnv()
    tags_wrapper = get_wrapper_fns("answer_tags")[0]
    ...
    random_action = wrapped_env.sample_random_action()
    assert random_action.startswith("<answer>") and random_action.endswith(
        "</answer>"
    ), (
